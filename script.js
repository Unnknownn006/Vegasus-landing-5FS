...
/* ============================================================
   ВТОРОЙ ЭКРАН — для вернувшегося посетителя.

   Первый визит: человек видит обычный лендинг, а в localStorage
   ложится отметка «был здесь». Любой следующий заход на страницу
   поднимает второй экран сразу — он и написан от лица того, кто
   уже был и ушёл: «You left them on the table».

   В первом визите экран тоже может всплыть, но только если
   человек собрался уходить: на десктопе курсор ушёл за верхнюю
   кромку окна, на тач-устройствах — резкий скролл вверх после
   прокрутки. Отключается флагом SHOW_ON_EXIT.

   Клик по кнопке экран НЕ гасит — наоборот, это и есть тот самый
   уход на сайт, после которого человек возвращается и должен
   увидеть напоминание.

   Закрыть экран нельзя: ни крестика, ни Escape, ни клика по
   затемнению. Вернувшийся видит только его.
   ============================================================ */
(() => {
  const el = document.getElementById("exit");
  if (!el) return;

  const SHOW_ON_RETURN = true;   // повторный заход — экран сразу
  const SHOW_ON_EXIT   = true;   // первый заход — по попытке уйти

  const VISITED_KEY = "vegasus:fs5:visited";
  const RETURN_DELAY = 900;      // дать странице отрисоваться
  const ARM_DELAY    = 6000;     // не дёргать сразу после загрузки
  const IDLE_SHOW    = 30000;    // страховка для тач-устройств
  ...
  const returning = !!read(VISITED_KEY);

  if (returning && SHOW_ON_RETURN) {
    timers.push(setTimeout(show, RETURN_DELAY));
  } else if (SHOW_ON_EXIT) {
    timers.push(setTimeout(() => { armed = true; }, ARM_DELAY));
    timers.push(setTimeout(show, IDLE_SHOW));
    document.addEventListener("mouseout", onMouseOut);
    window.addEventListener("scroll", onScroll, { passive: true });
  }

  /* отметку о визите ставим сразу — следующий заход уже «повторный» */
  if (!returning) write(VISITED_KEY, String(Date.now()));


  /* уход по кнопке — тоже визит: ставим отметку немедленно,
     не дожидаясь, пока браузер выгрузит страницу */
  document.querySelectorAll(".cta").forEach(a => {
    a.addEventListener("click", () => write(VISITED_KEY, String(Date.now())));
  });
})();
